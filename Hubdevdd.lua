...
local Libary = loadstring(game:HttpGet("https://raw.githubusercontent.com/lynix-dev/UI-library-/refs/heads/main/magnusuilib.txt"))()
...
local Paragraph = Tab1:AddParagraph({"Criadores", "the darknesxz"})
  ...
  Tab1:AddButton({
    ...
    Callback = function()
      ...
      setclipboard("https://www.tiktok.com/@darkzim.aepp?_t=ZM-8z75NNjT9yI&_r=1") -- Copia o link tambÃ©m, se quiser sÃ³ o @, remova esta linha
...
Tab2:AddTextBox({
    ...
    PlaceholderText = "ex: lo â†’ The Darknesxz",
...
Tab3:AddParagraph({
    Title = "vai ter mais coisas aqui na proxima atualizaÃ§ao",
    Content = ""
})
...
Tab4:AddParagraph({
    Title = "FunÃ§Ãµes para vocÃª usar em vocÃª",
    Content = ""
})
...
Tab5:AddButton({
    ...
    Description = "Teleporta o carro selecionado para sua posiÃ§Ã£o",
    Callback = function()
        if not _G.SelectedVehicle or _G.SelectedVehicle == "" then
            TeleportCarro:MostrarNotificacao("Nenhum carro selecionado!")
            return
...
Tab5:AddButton({
    ...
    Description = "Teleporta todos os carros do servidor para sua posiÃ§Ã£o",
    Callback = function()
        local originalPos
        if TeleportCarro.LocalPlayer.Character and TeleportCarro.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            originalPos = TeleportCarro.LocalPlayer.Character.HumanoidRootPart.Position
...
local function updateDropdown(dropdown, spectateToggle)
    pcall(function()
        ...
        elseif currentValue and table.find(playerNames, currentValue) then
            dropdown:Set(currentValue) -- MantÃ©m seleÃ§Ã£o se jogador ainda estÃ¡ no jogo
        end
    end)
end
...
-- FunÃ§Ã£o para teletransportar para o jogador selecionado (com ancoragem segura)
local function teleportToPlayer(playerName)
    ...
    if targetPlayer and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if not myHRP or not myHumanoid then
            print("Seu personagem nÃ£o estÃ¡ totalmente carregado para teletransporte.")
            return
        end

        -- Zerar a fÃ­sica do personagem antes do teleporte
        ...
        -- Teleportar para a posiÃ§Ã£o do jogador-alvo
        local success, errorMessage = pcall(function()
            myHRP.CFrame = CFrame.new(targetPlayer.Character.HumanoidRootPart.Position + Vector3.new(0, 2, 0)) -- Leve elevaÃ§Ã£o para evitar colisÃ£o com o chÃ£o
        end)
        if not success then
        ...
        -- Desancorar todas as partes do personagem e restaurar fÃ­sica
        for _, part in ipairs(LocalPlayer.Character:GetDescendants()) do
            if part:IsA("BasePart") then
                part.Anchored = false
                part.Velocity = Vector3.zero
        ...
        print("Jogador ou personagem nÃ£o encontrado para teletransporte.")
    end
end
...
local SpectateToggleTab10 = Tab9:AddToggle({
    ...
    Description = "Ativa/desativa a visualizaÃ§Ã£o do jogador selecionado",
    Default = false,
    Callback = function(state)
        if state then
...
-- RemoÃ§Ã£o automÃ¡tica de jogadores que saem
...
-- AtualizaÃ§Ã£o automÃ¡tica quando um novo jogador entra
...
local DropdownKillPullMethod = Tab9:AddDropdown({
    Name = "Selecionar MÃ©todo (Matar/Puxar)",
    Description = "Escolha o mÃ©todo para matar ou puxar",
    Options = {"SofÃ¡", "Ã”nibus"},
    Callback = function(value)
...
Tab9:AddButton({
    Name = "Matar",
    Description = "Inicia o matar com o mÃ©todo selecionado",
    Callback = function()
        ...
        if selectedKillPullMethod == "SofÃ¡" then
            killWithSofa(selectedPlayer)
        elseif selectedKillPullMethod == "Ã”nibus" then
...
Tab9:AddButton({
    Name = "Puxar",
    Description = "Inicia o puxar com o mÃ©todo selecionado",
    Callback = function()
        if isFollowingKill or isFollowingPull or running then return end
        if not selectedPlayer or not selectedKillPullMethod or selectedKillPullMethod ~= "SofÃ¡" then return end
...
local DropdownFlingMethod = Tab9:AddDropdown({
    Name = "Selecionar MÃ©todo de Fling",
    Description = "Escolha o mÃ©todo para aplicar fling",
    Options = {"SofÃ¡", "Ã”nibus", "Bola", "Bola V2", "Barco", "CaminhÃ£o"},
    Callback = function(value)
...
local function flingWithTruck(targetPlayer)
    ...
    -- Teletransporta para a posiÃ§Ã£o inicial do Ã´nibus para invocar o caminhÃ£o
    pcall(function()
        myHRP.Anchored = true
        myHRP.CFrame = CFrame.new(Vector3.new(1181.83, 76.08, -1158.83))
        task.wait(0.2)
    ...
    -- Desativa o cliente de carro para evitar interferÃªncias
    disableCarClient()

    -- Deleta qualquer veÃ­culo existente
    local args = { [1] = "DeleteAllVehicles" }
    pcall(function()
        ReplicatedStorage:WaitForChild("RE"):WaitForChild("1Ca1r"):FireServer(unpack(args))
    end)

    -- Invoca o caminhÃ£o (Semi) usando o comando fornecido
    ...
    -- Teletransporta para a posiÃ§Ã£o do assento do caminhÃ£o
    pcall(function()
        myHRP.Anchored = true
    ...
    -- Desativa a colisÃ£o das partes do caminhÃ£o e define a posse de rede
    ...
    flingConnection = RunService.Heartbeat:Connect(function()
        ...
        local verticalOffset = math.sin(tick() * 30) * 5 -- Oscila entre -5 e 5 unidades na vertical, ainda mais rÃ¡pido
        pcall(function()
            local targetPosition = newTargetHRP.Position + Vector3.new(0, verticalOffset, 0)
            trailer:PivotTo(CFrame.new(targetPosition)) -- Apenas movimento vertical, sem rotaÃ§Ã£o
        end)

        -- Verifica a distÃ¢ncia entre o trailer e o jogador-alvo para aplicar o fling
        ...
        -- Para o fling se o jogador-alvo estiver sentado ou apÃ³s 10 segundos
        local playerSeated = false
        ...
        if playerSeated or tick() - startTime > 10 then
            ...
            -- Teletransporta o caminhÃ£o para uma posiÃ§Ã£o fora do mapa
            pcall(function()
                truck:PivotTo(CFrame.new(Vector3.new(-59599.73, 2040070.50, -293391.16)))
            end)
            task.wait(0.5)

            -- Limpeza: Deleta o caminhÃ£o e reseta o jogador
            disableCarClient()
            local args = { [1] = "DeleteAllVehicles" }
            pcall(function()
                ReplicatedStorage:WaitForChild("RE"):WaitForChild("1Ca1r"):FireServer(unpack(args))
...
flingToggle = Tab9:AddToggle({
    Name = "Ativar Fling",
    Description = "Ativa ou desativa o fling com o mÃ©todo selecionado",
    Default = false,
    Callback = function(state)
        if state then
            ...
            if selectedFlingMethod == "SofÃ¡" then
            ...
            elseif selectedFlingMethod == "CaminhÃ£o" then
                flingWithTruck(selectedPlayer)
            elseif selectedFlingMethod == "Ã”nibus" then
...
-- VariÃ¡veis globais no inÃ­cio do Tab2
local Players = game:GetService("Players")
local LocalPlayer = Players.LocalPlayer
local Workspace = game:GetService("Workspace")
local ReplicatedStorage = game:GetService("ReplicatedStorage")
...
excludedPlayers = {} -- Tabela para jogadores excluÃ­dos dos flings

-- FunÃ§Ã£o auxiliar para obter a foto de perfil do jogador
...
-- FunÃ§Ã£o auxiliar para encontrar jogador por parte do nome
...
Tab9:AddTextBox({
    ...
    Callback = function(Value)
        if Value == "" then
            showNotification("Nenhuma AÃ§Ã£o", "Digite um nome para adicionar um jogador.", nil)
            return
        end

        local player = findPlayerByPartialName(Value)
        if player then
            -- Verifica se o jogador jÃ¡ estÃ¡ excluÃ­do
            for _, excluded in ipairs(excludedPlayers) do
                if excluded == player then
                    showNotification("Jogador JÃ¡ esta na whaitelist", "Jogador " .. player.Name .. " jÃ¡ foi adicionado.", getPlayerThumbnail(player.UserId))
                    return
                end
            end
            ...
            showNotification("Jogador NÃ£o Encontrado", "Nenhum jogador encontrado com '" .. Value .. "'.", nil)
        end
    end
})

-- BotÃ£o para verificar jogadores excluÃ­dos
Tab9:AddButton({"Verificar ExcluÃ­dos", function()
    if #excludedPlayers == 0 then
        showNotification("Nenhum na whaitelist", "Nenhum jogador estÃ¡ removido dos flings.", nil)
        return
    end
    for i, player in ipairs(excludedPlayers) do
        local thumbnail = getPlayerThumbnail(player.UserId)
        showNotification("Jogador adicionado " .. i, "Jogador " .. player.Name .. " estÃ¡ removido dos flings.", thumbnail)
        task.wait(0.5) -- Pequeno atraso entre notificaÃ§Ãµes para evitar sobreposiÃ§Ã£o
    end
end})

-- BotÃ£o para remover todos os jogadores excluÃ­dos
Tab9:AddButton({"Remover ExcluÃ­dos", function()
    if #excludedPlayers == 0 then
...
Tab9:AddButton({"Parar Tudo", function()
    ...
    soccerBall = nil
    originalProperties = nil
    showNotification("Tudo Parado", "Todas as funÃ§Ãµes foram desativadas.", nil)
end})
